...
function Login() {
  ...
  useEffect(() => {
    prefetchEncryptionKey().catch((error) => {
      console.error("Failed to prefetch encryption key:", error);
      setSubmitError("Unable to prepare secure login. Please refresh the page.");
    });
  }, []);
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const persistedUser = {
        ...
        fullname: userPayload.fullname || userPayload.full_name || userPayload.username || data.username,
        username: userPayload.username || data.username,
          ...
          <Form className="login-form" onSubmit={handleSubmit(onSubmit)} noValidate>
            <Form.Group className="login-field" controlId="username">
              ...
              {errors.username?.type === "pattern" && (
                <span className="error-text" style={{ color: "#f87171" }}>Must start with a letter; letters, numbers, and underscores only</span>
              )}
            </Form.Group>
            ...
            <button type="submit" className="login-button" disabled={isSubmitting || !isValid}>
