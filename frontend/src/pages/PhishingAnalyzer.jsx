...
import { addHistoryEntry, getHistory } from "../utils/history.js";
...
const RecentScans = ({ entries }) => {
  ...
  return (
    <section className="phish-card phish-card--history fade-in">
      <div className="phish-card__header recent-header">
        <div>
          <p className="eyebrow">Recent Scans</p>
          <h3 className="phish-card__title">Recent activity</h3>
          <p className="phish-card__description">Local-only log for quick recall. Click a card to review metadata.</p>
        </div>
        <div className="recent-legend">
          <span className="chip chip-muted">Local</span>
          <span className="chip chip-soft">Secure</span>
        </div>
      </div>
      <div className="recent-list">
        {entries.map((item, idx) => {
          const tone = toneForVerdict(item.status);
          return (
            <article key={idx} className="recent-card">
              <div className="recent-icon recent-icon--glow">
                <span aria-hidden="true">{recentIcon(tone)}</span>
              </div>
              <div className="recent-body">
                <div className="recent-title-row">
                  <div className="recent-title">{item.tool}</div>
                ...
                <div className="recent-meta">
                  ...
                  <span className="recent-risk">
                    Risk score <strong>{item.risk}</strong>
                  </span>
                </div>
              </div>
            </article>
...
function PhishingAnalyzer() {
  ...
  useEffect(() => {
    setHistoryEntries(getHistory().slice(0, 5));
  }, []);
  ...
  return (
    <>
      ...
      <main className="tool-page__content phish-page">
        ...
        <RecentScans entries={historyEntries} />
      </main>
    </>
  );
}
